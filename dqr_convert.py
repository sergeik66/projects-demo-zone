...
from pyspark.sql.functions import col, lit, row_number, to_json, collect_list, current_timestamp
...
def save_json_to_onelake(json_data, json_path):
    ...
    try:
        ...
        raise Exception(f"Error saving JSON to OneLake: {str(e)}")
...
def write_to_delta_table(spark, json_path, table_path, lakehouse_manager):
    ...
    try:
        if lakehouse_manager.check_if_table_exists(CONFIG["table_name"], CONFIG["schema_name"]):
            ...
            print(f"Successfully wrote to Delta table at {table_path}")
        else:
            print(f"Table {CONFIG['table_name']} does not exist in schema {CONFIG['schema_name']}")
    except Exception as e:
        raise Exception(f"Error writing to Delta table: {str(e)}")
...
def main():
    ...
    try:
        ...
        # Collect and validate JSON
        dq_json = json.loads(df_json.select(to_json(collect_list("dq_json"))).collect()[0][0])
        for idx in dq_json:
        ...
        print(f"DQ template conversion complete. JSON saved at: {paths['json_path']}")
        
        # Write to Delta table
        write_to_delta_table(spark, paths["json_path"], paths["table_path"], observability_lakehouse)
        
    except Exception as e:
        print(f"Error in DQ rule processing: {str(e)}")
        raise
    finally:
        spark.stop()
