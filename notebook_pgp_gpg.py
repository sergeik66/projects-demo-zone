...
use_gpg_fallback = False  # Set to True if GPG is installed in Fabric
...
def encrypt_files(input_folder: str, output_folder: str, pgp_enabled: bool) -> None:
    ...
    # List files in input folder (exclude .pgp/.gpg files)
    try:
    ...
    try:
        pgp = PGP(
            ...
            use_gpg_fallback=use_gpg_fallback
        )
    except Exception as e:
...
def decrypt_files(input_folder: str, output_folder: str, pgp_enabled: bool) -> None:
    ...
    for input_file in files:
        ...
        except (NotImplementedError, pgpy.errors.PGPError, ValueError) as e:
            logger.warning(f"Skipping {input_file}: Unsupported PGP/GPG format or passphrase error ({str(e)})")
            continue
        except Exception as e:
